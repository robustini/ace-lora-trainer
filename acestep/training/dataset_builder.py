...
def tiled_vae_encode(
    ...
    """Encode audio to VAE latents using tiled (chunked) encoding.

    For short audio (<= chunk_seconds) this is identical to a single vae.encode().
    For longer audio the waveform is split into overlapping windows; each window
    is encoded independently and only the *core* (non-overlap) latent frames are
    kept.  This keeps VRAM usage bounded regardless of audio length.

    Strategy: overlap-discard (NOT blending).  The overlap regions provide
    context so that the convolutions inside the VAE don't produce edge
    artifacts, but the overlap portion of the output is thrown away.

    Args:
        vae:  The AutoencoderOobleck (or compatible) VAE.
        audio: Tensor of shape [B, C, T] (batch, channels, time samples)
               already on the correct device and dtype.
        sample_rate: Audio sample rate (default 48000 for ACE-Step).
        chunk_seconds: Length of each encoding window in seconds.
        overlap_seconds: Overlap on each side of the core region.

    Returns:
        Latent tensor [B, 64, T_latent] (same convention as vae.encode).
    """
    B, C, S = audio.shape
    chunk_size = int(sample_rate * chunk_seconds)
    overlap = int(sample_rate * overlap_seconds)

    # Short audio — single-pass encoding
    if S <= chunk_size:
        return vae.encode(audio).latent_dist.sample()

    # Long audio — tiled encoding
    stride = chunk_size - 2 * overlap  # core size per chunk
    num_chunks = math.ceil(S / stride)

    logger.info(
        f"Tiled VAE encode: {S / sample_rate:.1f}s audio → "
        f"{num_chunks} chunks ({chunk_seconds}s window, {overlap_seconds}s overlap)"
    )

    encoded_cores: List[torch.Tensor] = []
    ...
    for i in range(num_chunks):
        ...
        # Expand to window (core + overlap on each side)
        win_start = max(0, core_start - overlap)
        win_end = min(S, core_end + overlap)

        chunk = audio[:, :, win_start:win_end]
        latent_chunk = vae.encode(chunk).latent_dist.sample()  # [B, 64, T_chunk]

        # Compute downsample factor from the first chunk
        if downsample_factor is None:
            downsample_factor = chunk.shape[-1] / latent_chunk.shape[-1]

        # Calculate how many latent frames to trim on each side
        trim_left = int(round((core_start - win_start) / downsample_factor))
        trim_right = int(round((win_end - core_end) / downsample_factor))

        end_idx = latent_chunk.shape[-1] - trim_right if trim_right > 0 else latent_chunk.shape[-1]
        core_latent = latent_chunk[:, :, trim_left:end_idx]
        encoded_cores.append(core_latent)

    final = torch.cat(encoded_cores, dim=-1)  # [B, 64, T_total]
    logger.info(f"Tiled VAE encode complete: {num_chunks} chunks → latent shape {list(final.shape)}")
    return final
...
class DatasetBuilder:
    ...
    def get_samples_dataframe_data(self) -> List[List[Any]]:
        ...
        for i, sample in enumerate(self.samples):
            ...
            elif sample.lyrics and sample.lyrics.strip() and sample.lyrics != "[Instrumental]":
                lyrics_status = "🎤"  # Has lyrics from captioner JSON
    ...
    def preprocess_to_tensors(
        ...
        for i, sample in enumerate(labeled_samples):
            try:
                ...
                # Uses tiled encoding for long audio (>30s) to bound VRAM usage
                with torch.no_grad():
                    latent = tiled_vae_encode(vae, audio)
        ...
        # Build per-sample info for manifest (tensor shapes, dtype, file sizes)
        import hashlib
        from datetime import datetime
        ...
        for p in output_paths:
            ...
            try:
                ...
                entry["shapes"] = {
                    k: list(v.shape) for k, v in data.items() if isinstance(v, torch.Tensor)
                }
                entry["dtype"] = str(next(
                    (v.dtype for v in data.values() if isinstance(v, torch.Tensor)), "unknown"
                ))
                # MD5 checksum for data integrity verification
                with open(p, "rb") as bf:
                    entry["md5"] = hashlib.md5(bf.read()).hexdigest()
                # Copy sample metadata if present
                if "metadata" in data and isinstance(data["metadata"], dict):
                    entry["sample_metadata"] = data["metadata"]
            except Exception as e:
                logger.warning(f"Failed to read tensor info for manifest: {p}: {e}")
